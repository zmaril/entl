    ...
    GitWorld {
        commits: vec![
            GenCommit {
                parents: vec![],
                tree: t0,
                author: sig(1_000_000_000),
                committer: sig(1_000_000_000),
                message: "first\n".into(),
            },
            GenCommit {
                parents: vec![0],
                tree: t1,
                author: sig(1_000_000_100),
                committer: sig(1_000_000_100),
                message: "second\n".into(),
            },
        ],
        refs: vec![
...
fn materializes_valid_deterministic_repo() {
    ...
    assert_eq!(
        git(d1.path(), &["rev-parse", "refs/heads/main"])
            .unwrap()
            .trim(),
        oids[1]
    );
    assert_eq!(
        git(d1.path(), &["rev-parse", "refs/tags/v1"])
            .unwrap()
            .trim(),
        oids[0]
    );
    assert_eq!(
        git(d1.path(), &["rev-list", "--count", "refs/heads/main"])
            .unwrap()
            .trim(),
        "2"
    );
