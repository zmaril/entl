...
//! This is the ONE place engine wiring lives — the PyO3 surface (classes,
//! GIL-released methods, iterator dressing) is generated from the fluessig
//! catalog. (Structurally identical to entl-node's core_impl — the trait types
//! are per-crate; deduplicating them is a noted follow-up.)
...
fn select_of(
    ...
    entl_core::SinkSelect {
        ...
        rename: rename
            .map(|rs| rs.into_iter().map(|r| (r.from_, r.to)).collect())
            .unwrap_or_default(),
        schema,
    }
}
