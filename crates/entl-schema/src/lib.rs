...
//! entl-schema — entl's COMPLETE catalog (all 28 tables + the full binding op
//! surface) authored with the `fluessig` **Rust derive front end**. This crate is
//! the single source of truth for entl's schema; the emit bins
//! (`src/bin/fluessig-emit.rs` / `fluessig-emit-api.rs`) print the `catalog.json` /
//! `api.json` that `scripts/gen.sh` feeds to `fluessig-gen`, exactly where the
//! TypeSpec `entl.tsp` + Node emitter used to sit.
//!
//! To change entl's data model or op surface, edit the `#[derive(Entity)]` /
//! `#[derive(Edge)]` / `#[fluessig::export]` items below and re-run
//! `scripts/gen.sh` (or `bun run gen` in `crates/entl-node`), then commit the
//! regenerated artifacts.
//!
//! The front end's capabilities this schema exercises:
//!
...
pub struct CommitParent {
    ...
    // PROPOSED: @key on an edge-struct field = local key; edge PK = source key + local key (FINDINGS #3)
    #[fluessig(key)]
    pub idx: i32,
}
...
// NOTE: the committed schema_docs.json truncates this desc at the `@compose`
// token — a quirk of the retired TypeSpec emitter. Reproduced verbatim below for
// byte-exact parity; carrying the full sentence ("… *also* be a @compose child
// of Commit (embed in doc stores) is an open modeling tension recorded in
// FINDINGS #5.") is a doc-only follow-up.
