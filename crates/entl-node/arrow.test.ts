...
test("changes() batches decode with tableFromIPC and agree with the JSON plane", async () => {
  ...
  for (
    ...
    rowsByTable.set(
      batch.table,
      (rowsByTable.get(batch.table) ?? 0) + table.numRows,
    );
    expect(["insert", "update", "upsert", "delete", "replace"]).toContain(
      batch.op,
    );
  ...
  const viaJson = JSON.parse(
    await entl.query("SELECT count(*)::int AS n FROM commits"),
  );
...
test("queryArrow returns one IPC stream the JS Arrow lib decodes, native types included", async () => {
  ...
  const t = tableFromIPC(
    await entl.queryArrow("SELECT oid, author_name, is_merge FROM commits"),
  );
  ...
  const empty = tableFromIPC(
    await entl.queryArrow("SELECT 1 AS x WHERE false"),
  );
