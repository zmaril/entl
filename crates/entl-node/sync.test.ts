...
import { EntlTables, syncInto } from "./sync.ts";
...
test("syncInto mirrors entl's DuckDB into a live PGlite via the Rust driver plan", async () => {
  ...
  const commits = await pg.query<{ n: number }>(
    'SELECT count(*)::int AS n FROM "entl"."commits"',
  );
  ...
  const again = await pg.query<{ n: number }>(
    'SELECT count(*)::int AS n FROM "entl"."commits"',
  );
...
test("rename + table selection flow through to the plan", async () => {
  ...
  expect(counts.commits).toBe(2);
  const renamed = await pg.query<{ n: number }>(
    'SELECT count(*)::int AS n FROM "mirror"."git_commits"',
  );
