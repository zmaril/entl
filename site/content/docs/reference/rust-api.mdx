...
impl Db {
    ...
    fn query_arrow_ipc(&self, sql: &str) -> Result<Vec<u8>>;
}
```
...
```rust
struct DriverSink<E: FnMut(Statement) -> Result<()>> {
    emit: E,
    dialect: Dialect,
    select: SinkSelect,
...
```rust
fn backfill(conn: &duckdb::Connection, sink: &mut dyn Sink, tables: &[&str]) -> Result<u64>
```
...
```rust
fn driver_tables() -> Vec<&'static str>
```
...
```rust
fn statement_channel(capacity: usize) -> (Sender<Statement>, StatementStream)
```
...
```rust
fn batch_ipc(batch: &RecordBatch) -> anyhow::Result<Vec<u8>>
```
