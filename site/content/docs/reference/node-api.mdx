...
A driver-sink statement plan — the "mirror into a database entl-core doesn't link" surface (notes/design/multidb.md). All the DDL / type-mapping / upsert logic lives in Rust core (`DriverSink`); this streams the resulting `{sql, params}` statements so the JS side only executes them against its own client (e.g. PGlite: `await pg.query(sql, params)`). `next()` resolves to the next statement or `null` when the plan is complete.
